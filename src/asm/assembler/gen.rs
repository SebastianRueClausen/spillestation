...
use super::parse::{Ir, IrAddr, IrTy};
use super::Error;
...
impl<'a> CodeGen<'a> {
    ...
    fn resolve_label(&self, line: usize, addr: &IrAddr) -> Result<u32, Error> {
        match addr {
            IrAddr::Label(id) => match self.labels.get(id) {
                Some(addr) => Ok(*addr),
                None => Err(
                    Error::new(line, format!("Unresolved symbol '{}'", id))
                ),
            }
            IrAddr::Abs(addr) => Ok(*addr),
        }
    }

    fn branch_offset(&self, line: usize, addr: &IrAddr) -> Result<i32, Error> {
        let loc = self.code.len() as i32 + 4;
        let dest = self.resolve_label(line, addr)? as i32;
        Ok((dest - loc) >> 2)
    }

    fn jump_addr(&self, line: usize, addr: &IrAddr) -> Result<u32, Error> {
