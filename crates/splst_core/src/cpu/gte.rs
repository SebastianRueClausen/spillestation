...
impl Gte {
    pub(super) fn control_store(&mut self, offset: u32, val: u32) {
        ...
        match offset {
            ...
            31 => {
                self.control.flags.0 = val & 0x7ff_ff000;
                self.control.flags.update_error_flag();
            }
            _ => unsafe {
    ...
    fn rtp(&mut self, vec: Vec3<i16>, shift: u8, clamp: bool) -> i64 {
        ...
        let mut dot_row = |add: i32, row: usize| {
            ...
            let v3 = sign_extend_mac(&mut self.control.flags, 2, v2 + v3);
    ...
    fn ncd(&mut self, vec: Vec3<i16>, shift: u8, clamp: bool) {
        ...
        (self.data.mac, self.data.ir) = mat_mul(
            &self.control.lcm,
            &self.control.bk,
            &self.ir_vec(),
            &mut self.control.flags,
...
fn mat_mul(
    ...
    let mut mul_row = |add: i32, row: usize| {
        ...
        let v2 = i64::from(mat.0[row].y) * i64::from(vec.y);
        let v3 = i64::from(mat.0[row].z) * i64::from(vec.z);

        let v1 = sign_extend_mac(flags, row, v0 + v1); 
        let v2 = sign_extend_mac(flags, row, v1 + v2); 
        let v3 = sign_extend_mac(flags, row, v2 + v3); 

        let mac = saturate_to_mac(flags, row, shift, v3);
...
impl Flags {
    ...
    fn update_error_flag(&mut self) {
        self.0 = self.0.set_bit(31, self.0 & 0x7f87_e000 != 0)
    }
